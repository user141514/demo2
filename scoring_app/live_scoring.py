...
9. 输出风格对齐正式评估报告：禁止使用“表现较强”“支撑充分”“建议补充证据”“已有可保留线索”等模板句；多写具体对象，例如战略背景、RACI/5WHY/逻辑树等工具、百分比/天数/阶段数据、跨部门角色、现场表达节奏，并优先保留提升、下降、周期变化等结果型数据。
...
12. 每条 overall_comment、strengths、improvements、evidence、comment 都必须尽量绑定至少一个作业信号，例如“RACI 分工”“52天降至46天”“38%→75%→80%”“ERP库存数据”“语速偏快”；如果材料没有对应信号，要明确写出“缺少具体案例/缺少量化数据/缺少责任分工/缺少资源规划/缺少复盘证据”（即缺少案例/数据/反思/资源规划），不得虚构。

优秀输出示例（只学习写法，不要照抄示例内容）：
优秀 evidence 示例：
优势亮点：学员运用SWOT工具识别个人发展核心问题，没有停留在表面技能短板，而是指出“主动挖掘业务痛点不足”和“团队管理停留在分任务、审报告阶段”等关键矛盾，并结合LTP中心成本管控场景进行自我剖析，说明其问题识别已能连接真实业务挑战。不足在于缺少人才测评工具或第三方反馈数据，削弱了自我判断的客观性。

优秀 comment 示例：
改进空间：建议补充商业综合推理或管理能力测评结果，将各维度得分与当前能力短板交叉印证；同时引入360度反馈或业务部门评价，避免只依赖自我陈述，使问题诊断更客观。

请模仿上述示例的写法，但不要照抄示例内容。每条 evidence 控制在 2-3 句，必须同时包含“具体证据 + 评价判断 + 不足边界”；每条 comment 控制在 1-2 句，只写可执行改进建议。可以保留“行动目标符合业务场景”这类业务判断，但禁止输出“符合评分标准”“属于某某水平”“达到某某档位”等评分标准复述。
